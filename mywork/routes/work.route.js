...
router.post('/',  verifyToken, (req, res) => {       /// restricting the api with jwt verification 

    jwt.verify(req.token, 'secretkey', (err, authData) =>{      // hardcoded the "secretkey" which can be vulnerable
        if(err) {
            res.sendStatus(403)
        } else {
...
router.delete('/:id', async (req, res) => {
    const result = await Work.destroy({        // deleting the table with the id 
...
return res.sendStatus(200)
 ...
 router.put('/:id', async (req, res) => {
    const result = await Work.update({       // sample update request 
